...
// Create transporter optimized for Vercel serverless
const createTransporter = () => {
  console.log('Creating Gmail transporter for Vercel serverless...')
  ...
  return nodemailer.createTransport({
    ...
    requireTLS: true,
    debug: true, // Enable debug for Vercel logs
  })
}
...
export async function sendPasswordResetEmail(email, resetToken) {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Email timeout after 40 seconds')), 40000)
    )
